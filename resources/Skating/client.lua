...
RegisterNetEvent("skating:roller")
AddEventHandler("skating:roller", function(item)
	-- print("command spawn
	if not changing then
		changing = true
		if not haveRoller then
			exports.globals:notify("Putting skates on...", "INFO: Use /removeskates to take your skates off")
			putOnRollers(item)
		else
			removeSkates()
			putOnRollers(item)
		...
		TriggerEvent("usa:notify", "You are already changing into/out of skates!")
	end
end)

RegisterNetEvent("skating:iceroller")
AddEventHandler("skating:iceroller", function(item)
	-- print("command spawn
	if not changing then
		changing = true
		if not haveRoller then
...
RegisterNetEvent("skating:leave")
AddEventHandler("skating:leave", function()
	if haveRoller then
		removeSkates()
	end
end)
...
function removeSkates()
	TriggerServerEvent("skates:addItem", currentType)
	currentType = nil
	haveRoller = false
	actif = false
	Citizen.Wait(1500)
